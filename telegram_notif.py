...
def notify_scan(equity: float, regime: str, n_open: int, n_pending: int,
                n_signals: int, n_closed: int, next_scan_str: str,
                bear_s_active: bool = False):
    """Notif horaire : résumé du scan."""
    ret_pct  = (equity - INITIAL_CAPITAL) / INITIAL_CAPITAL * 100
    sign     = "+" if ret_pct >= 0 else ""
    r_emoji  = "🐻" if "BEAR" in regime.upper() else ("🔴" if "CHAOS" in regime.upper() else "🐂")
    s_note   = "  · Pattern S actif" if bear_s_active else ""

    if n_signals > 0:
        activity = f"🎯 {n_signals} signal{'s' if n_signals>1 else ''} détecté{'s' if n_signals>1 else ''}"
    elif n_closed > 0:
        activity = f"📊 {n_closed} trade{'s' if n_closed>1 else ''} fermé{'s' if n_closed>1 else ''}"
    else:
        activity = "😴 Aucun signal ce scan"

    text = (
        f"⏱ <b>Scan PRISM v33 — {__import__('datetime').datetime.now().strftime('%H:%M')}</b>\n"
        f"💰 Capital : <b>€{equity:,.2f}</b> ({sign}{ret_pct:.1f}%)\n"
        f"{r_emoji} Régime : {regime}{s_note}\n"
        f"📌 {n_open} position{'s' if n_open!=1 else ''} · {n_pending} en attente\n"
        f"{activity}\n"
        f"<i>Prochain scan : {next_scan_str}</i>"
    )
    _send(text)


def notify_regime_change(old_regime: str, new_regime: str,
                         btc_price: float, adx: float):
    """Changement de régime macro BTC (bear↔bull↔range)."""
    icons = {"BEAR": "🐻", "BULL": "🐂", "RANGE": "↔️", "CHAOS": "🌪️"}
    old_i = icons.get(old_regime, "?")
    new_i = icons.get(new_regime, "?")
    ctx = {
        ("BEAR", "BULL"): ("Macro bearish terminé", "Pattern S suspendu. En attente de setups C/D."),
        ("BULL", "BEAR"): ("Retournement baissier", "Pattern S activé. Rebonds à shorter sur 26 altcoins."),
        ("BEAR", "RANGE"): ("Fin du bear, transition range", "ADX faible — Pattern R prioritaire."),
        ("BULL", "RANGE"): ("Bull s'essouffle", "ADX sous 22 — Range actif, Pattern R."),
        ("RANGE", "BULL"): ("Breakout haussier", "Momentum montant — C/D longs en alerte."),
        ("RANGE", "BEAR"): ("Breakdown baissier", "Pattern S en surveillance — gate bear_macro bientôt actif."),
    }
    headline, tip = ctx.get((old_regime, new_regime),
                             ("Changement de régime", "Ajuster les attentes de signal."))
    text = (
        f"🔔 <b>RÉGIME CHANGÉ — PRISM v33</b>\n"
        f"{old_i} {old_regime} → {new_i} <b>{new_regime}</b>\n"
        f"BTC : <code>{btc_price:,.0f}$</code>  ADX : {adx:.1f}\n"
        f"<b>{headline}</b>\n"
        f"<i>{tip}</i>"
    )
    _send(text)
...
def notify_readiness(sym: str, pattern: str, readiness_pct: int,
                     detail: str, side: str = "long"):
    """Alerte pré-signal : pattern proche de déclencher (readiness > 75%)."""
    direction = "hausse 📈" if side == "long" else "baisse 📉"
    pair = sym.replace("-USDT", "")
    bar_fill = "█" * (readiness_pct // 10) + "░" * (10 - readiness_pct // 10)
    text = (
        f"👀 <b>Setup en approche — Pattern {pattern}</b>\n"
        f"<b>{pair}</b> — {direction}\n"
        f"Maturité : {bar_fill} {readiness_pct}%\n"
        f"<i>{detail}</i>\n"
        f"<i>Pas encore un signal — surveillance active</i>"
    )
    _send(text)
